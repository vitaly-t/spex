 ...
 * - `error` = the error thrown or the rejection reason
 * - `dest` = resolved {@link batch} that was passed into the function
 *
 ...
 *  - *Normal Reject*: when one of the pages failed to resolve as a {@link batch}
 *  - *Internal Reject*: caused by either the `source` or the `dest` functions
 *
 * *Normal Rejects* are reported with object `{index, data}`:
 *  - `index` = index of the page rejected by method {@link batch}
 *  - `data` = the rejection data from method {@link batch}
 *
 ...
 * For *Normal Rejects* it will return `data.first` (see method {@link batch}),
