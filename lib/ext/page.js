 ...
 *
 * Expected to return a $[mixed value] that resolves with either `undefined` or the
 * next page of data (array of $[mixed values]) to be resolved. If the returned value
 * resolves into `undefined` or an empty array, it signals the end of the sequence,
 * and the method resolves.
 *
 ...
 *
 * If the returned $[mixed value] resolves into anything other than an array or `undefined`,
 * the method will reject with object `{index, error}`:
 ...
 * @param {function} [dest]
 *
 * Optional destination function (notification callback), to receive resolved $[batch] of data
 ...
 * And if a promise is returned, the method will not request the next page from the `source`
 ...
 * been reached.
 *
 * By default, the sequence is unlimited, and will continue till the `source` function returns
 * `undefined` or an empty array, or when an error/reject occurs.
 *
 * @returns {Promise}
 *
 ...
 *
 * Object for both types of the rejects comes with method `getError()` to simplify access
 * to the error. For **Normal Rejects** it will return `data.getErrors()[0]` (see method $[batch]),
 * and `error` value for **Processing Rejects**.
 *
 * @see $[batch]
 */
