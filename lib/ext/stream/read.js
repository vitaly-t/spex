 ...
 * Sets the read size from the stream's buffer when the next data is available.
 * By default, the method uses as few reads as possible to get all the data
 * currently available in the stream's buffer.
 *
 * @returns {Promise}
...
function _read(stream, receiver, closable, readSize) {
    if ($utils.hasProperty(closable, ['closable', 'readSize'])) {
