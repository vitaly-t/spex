 ...
 * Expected to return the next $[mixed value] to be resolved. Returning nothing (`undefined`)
 * signals the end of the sequence.
 *
 * Parameters:
 *  - `index` - current request index in the sequence
 *  - `data` - resolved data from the previous call to the function (`undefined` when `index=0`)
 *
...
function sequence(source, dest, limit, track) {
    ...
    return $p(function (resolve, reject) {
        ...
        function loop(idx) {
            $utils.resolve.call(self, source, [idx, data], function (value, async) {
                data = value;
                if (data === undefined) {
                    finish();
                } else {
